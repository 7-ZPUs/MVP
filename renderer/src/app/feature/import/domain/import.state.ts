...
export class ImportState {
  ...
  readonly selectedDocument: Signal<DipTreeNode | null> = computed(
    () => this._state().selectedDocument,
  );
  ...
  readonly indicizzazione: Signal<StatoIndicizzazione> = computed(
    () => this._state().indicizzazione,
  );
  readonly nodeCache: Signal<Map<string, DipTreeNode[]>> = computed(() => this._state().nodeCache);
  readonly nodeChildrenErrors: Signal<Map<string, AppError>> = computed(
    () => this._state().nodeChildrenErrors,
  );

  /** UC-6 */
  isEmpty(): boolean {
    return this._state().phase === ImportPhase.EMPTY;
  }
  ...
  getSnapshot(): ImportStateSnapshot {
    return this._state();
  }
