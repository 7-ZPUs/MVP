...
const meta = [new Metadata("autore", "Mario Rossi"), new Metadata("anno", "2024", "number")];
...
describe("Document entity", () => {
    ...
    describe("fromDB", () => {
        it("ricostruisce correttamente da una riga del database", () => {
            const row: DocumentRow = { id: 10, uuid: "doc-uuid", integrityStatus: "VALID", processId: 3 };
    ...
    describe("toDTO", () => {
        it("lancia un errore se id è null", () => {
            const doc = new Document("uuid", [], 1);
            expect(() => doc.toDTO()).toThrow("Cannot convert to DTO: Document entity is not yet persisted and has no ID.");
        });

        it("restituisce il DTO corretto", () => {
            const row: DocumentRow = { id: 2, uuid: "doc-abc", integrityStatus: "INVALID", processId: 5 };
            const doc = Document.fromDB(row, meta);
