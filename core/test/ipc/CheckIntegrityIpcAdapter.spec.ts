...
function getRegisteredHandler(
  handleMock: ReturnType<typeof createIpcMainMock>["handleMock"],
  channel: string,
): IpcHandler {
...
describe("CheckIntegrityIpcAdapter", () => {
  ...
  it("TU-S-browsing-107: register() should resolve all integrity use-cases and register all integrity channels", () => {
    ...
    expect(
      handleMock.mock.calls.some(
        (call) => call[0] === IpcChannels.CHECK_DIP_INTEGRITY_STATUS,
      ),
    ).toBe(true);
  });
  ...
  it("TU-S-browsing-108: register() should delegate non-file integrity checks to the resolved use-cases", async () => {
    ...
    checkDocumentIntegrityUC.execute.mockResolvedValue(
      IntegrityStatusEnum.VALID,
    );
    checkProcessIntegrityUC.execute.mockResolvedValue(
      IntegrityStatusEnum.INVALID,
    );
    checkDocumentClassIntegrityUC.execute.mockResolvedValue(
    ...
    const dipResult = await getRegisteredHandler(
      handleMock,
  ...
  it("TU-S-browsing-109: register() should await and return file integrity result", async () => {
    ...
    const result = await getRegisteredHandler(
      handleMock,
  ...
  it("TU-S-browsing-110: register() should propagate rejection from file integrity use-case", async () => {
    const { ipcMain, handleMock } = createIpcMainMock();
    checkFileIntegrityUC.execute.mockRejectedValue(
      new Error("File with id 99 not found"),
    );

    CheckIntegrityIpcAdapter.register(ipcMain);

    await expect(
      getRegisteredHandler(handleMock, IpcChannels.CHECK_FILE_INTEGRITY_STATUS)(
        {},
        99,
      ),
