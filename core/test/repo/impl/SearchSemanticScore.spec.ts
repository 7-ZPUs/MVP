...
type SemanticEntry = { document: ReturnType<typeof makeDocument>; score: number };

const buildPorts = (entries: SemanticEntry[]) => {
  const documentRepo: IGetDocumentByIdPort = { getById: vi.fn() };
  const vectorRepo: ISearchSimilarVectorsPort = { searchSimilarVectors: vi.fn() };

  (vectorRepo.searchSimilarVectors as ReturnType<typeof vi.fn>).mockResolvedValue(
    entries.map((entry, index) => ({ documentId: index, score: entry.score })),
  );
  (documentRepo.getById as ReturnType<typeof vi.fn>).mockImplementation(
    (documentId: number) => entries[documentId]?.document ?? null,
  );

  return { documentRepo, vectorRepo };
};
...
describe("SearchSemanticUC — calcolo score", () => {
  ...
  it("score è 1.0 quando la distanza è 0 (corrispondenza perfetta)", async () => {
    const { documentRepo, vectorRepo } = buildPorts([
  ...
  // Il limite massimo di risultati è 10
  it("non restituisce più di 10 risultati", async () => {
    const docs = Array.from({ length: 10 }, (_, i) => ({
      document: makeDocument(`uuid-${i}`, `doc${i}.pdf`),
      score: 1 - i * 0.05,
    }));
    const { documentRepo, vectorRepo } = buildPorts(docs);
