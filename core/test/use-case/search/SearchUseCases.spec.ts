...
describe("SearchDocumentsUC", () => {
  ...
  it("mappa correttamente uuid, nome e tipoDocumento nel SearchResult", () => {
    ...
    const results = uc.execute(emptyFilters);
  ...
  it("ritorna array vuoto se il repository non trova risultati", () => {
  ...
  it("usa stringa vuota per name e type se i metadati sono assenti", () => {
  ...
  it("passa i filtri al repository senza modificarli", () => {
    (repo.searchDocument as ReturnType<typeof vi.fn>).mockReturnValue([]);
  ...
  it("mappa correttamente più documenti restituiti dal repository", () => {
    const docs = [
  ...
  it("ignora metadati con chiavi diverse da nome e tipoDocumento", () => {
  ...
  it("usa il primo metadato corrispondente quando ci sono duplicati (nome/tipoDocumento)", () => {
...
describe("SearchSemanticUC", () => {
  let documentRepo: Pick<IGetDocumentByIdPort, "getById">;
  let vectorRepo: Pick<ISearchSimilarVectorsPort, "searchSimilarVectors">;
  ...
  it("ritorna coppie documento+score dal repository", async () => {
    ...
    ).mockResolvedValue([{ documentId: 21, score: 0.92 }]);
    (documentRepo.getById as ReturnType<typeof vi.fn>).mockReturnValue(doc);

    const uc = new SearchSemanticUC(
      documentRepo as IGetDocumentByIdPort,
      vectorRepo as ISearchSimilarVectorsPort,
      aiAdapter,
    );
  ...
  it("lo score semantico è sempre un numero, mai null", async () => {
    ...
    (
      vectorRepo.searchSimilarVectors as ReturnType<typeof vi.fn>
    ).mockResolvedValue([{ documentId: 22, score: 0.75 }]);
    (documentRepo.getById as ReturnType<typeof vi.fn>).mockReturnValue(doc);
  ...
  it("preserva l'ordine dei risultati restituito dal repository", async () => {
    const docHigh = makeDocument(31, "uuid-high", [
      { name: "nome", value: "alto.pdf" },
    ]);
    const docMid = makeDocument(32, "uuid-mid", [
      { name: "nome", value: "medio.pdf" },
    ]);
    const docLow = makeDocument(33, "uuid-low", [
  ...
  it("ritorna comunque documento e score anche con metadati assenti", async () => {
    ...
    (
      vectorRepo.searchSimilarVectors as ReturnType<typeof vi.fn>
    ).mockResolvedValue([{ documentId: 43, score: 0.88 }]);
    (documentRepo.getById as ReturnType<typeof vi.fn>).mockReturnValue(doc);
  ...
  it("preserva il documento originale anche con metadati duplicati", async () => {
    ...
    (
      vectorRepo.searchSimilarVectors as ReturnType<typeof vi.fn>
    ).mockResolvedValue([{ documentId: 44, score: 0.5 }]);
    (documentRepo.getById as ReturnType<typeof vi.fn>).mockReturnValue(doc);
