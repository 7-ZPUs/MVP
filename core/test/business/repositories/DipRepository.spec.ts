...
describe("DipRepository", () => {
  ...
  // identifier: TU-F-B-27
  // method_name: save()
  // description: save crea un dip con status UNKNOWN
  ...
  // identifier: TU-F-B-110
  // method_name: getById()
  // description: getById e getByUuid restituiscono l'entità
  ...
  // identifier: TU-F-B-28
  // method_name: updateIntegrityStatus()
  // description: updateIntegrityStatus e getByStatus funzionano
  ...
  // identifier: TU-F-B-29
  // method_name: ritorna()
  // description: ritorna array vuoto se nessuna riga trovata per getByStatus
  // expected_value: matches asserted behavior: ritorna array vuoto se nessuna riga trovata per getByStatus
  it("ritorna array vuoto se nessuna riga trovata per getByStatus", () => {
    db.prepare.mockReturnValueOnce({
      all: vi.fn().mockReturnValue([]),
    });
    const found = repo.getByStatus(IntegrityStatusEnum.VALID);
  ...
  // identifier: TU-F-B-30
  // method_name: save()
  // description: save fallback if changes is 0
  // expected_value: matches asserted behavior: save fallback if changes is 0
  it("save fallback if changes is 0", () => {
    db.prepare
      .mockReturnValueOnce({
  ...
  // identifier: TU-F-B-31
  // method_name: save()
  // description: save fallback if lastInsertRowid is 0 but changes > 0
  // expected_value: matches asserted behavior: save fallback if lastInsertRowid is 0 but changes > 0
  it("save fallback if lastInsertRowid is 0 but changes > 0", () => {
    db.prepare
      .mockReturnValueOnce({
