...
describe("Process use-cases", () => {
    it("CreateProcessUC delega a repo.save", () => {
        ...
        const input = { documentClassId: 1, uuid: "proc-1", metadata: [] };
        const result = uc.execute(input);

        const savedEntity = (repo.save as ReturnType<typeof vi.fn>).mock.calls[0][0] as Process;

        expect(repo.save).toHaveBeenCalledTimes(1);
        expect(savedEntity).toBeInstanceOf(Process);
        expect(savedEntity.getDocumentClassId()).toBe(input.documentClassId);
    ...
    it("CheckProcessIntegrityStatusUC aggiorna lo status aggregato dai document", () => {
        const processRepo: Pick<IProcessRepository, "getById" | "updateIntegrityStatus"> = {
            getById: vi.fn().mockReturnValue(new Process(1, "proc-1", [])),
            updateIntegrityStatus: vi.fn(),
        };
        const docRepo: Pick<IDocumentRepository, "getAggregatedIntegrityStatusByProcessId"> = {
            getAggregatedIntegrityStatusByProcessId: vi.fn().mockReturnValue(IntegrityStatusEnum.VALID),
        };

        const uc = new CheckProcessIntegrityStatusUC(processRepo as IProcessRepository, docRepo as IDocumentRepository);
        const result = uc.execute(1);

        expect(docRepo.getAggregatedIntegrityStatusByProcessId).toHaveBeenCalledWith(1);
        expect(processRepo.updateIntegrityStatus).toHaveBeenCalledWith(1, IntegrityStatusEnum.VALID);
