...
const makeDocument = (uuid: string, metadata: { name: string; value: string }[] = []) => {
    ...
    return Document.fromDB(
        { id: 1, uuid, integrityStatus: 'UNKNOWN', processId: 1 },
        meta as any
    );
};
...
describe('SearchDocumentsUC', () => {
    ...
    it('mappa correttamente più documenti restituiti dal repository', async () => {
        ...
        expect(results.map(r => r.documentId)).toEqual(['uuid-a', 'uuid-b', 'uuid-c']);
...
describe('SearchSemanticUC', () => {
    ...
    // La query deve essere passata intatta al repository
    it('passa la query al repository senza modificarla', async () => {
        (repo.searchDocumentSemantic as ReturnType<typeof vi.fn>).mockResolvedValue([]);

        const uc = new SearchSemanticUC(repo as IDocumentRepository);
        await uc.execute('ricerca semantica avanzata');

        expect(repo.searchDocumentSemantic).toHaveBeenCalledWith('ricerca semantica avanzata');
    });

    // Più risultati ordinati per score decrescente
    it('preserva l\'ordine dei risultati restituito dal repository', async () => {
        const docs = [
            { document: makeDocument('uuid-high',  [{ name: 'nome', value: 'alto.pdf'  }]), score: 0.95 },
            { document: makeDocument('uuid-mid',   [{ name: 'nome', value: 'medio.pdf' }]), score: 0.78 },
            { document: makeDocument('uuid-low',   [{ name: 'nome', value: 'basso.pdf' }]), score: 0.61 },
        ];
        (repo.searchDocumentSemantic as ReturnType<typeof vi.fn>).mockResolvedValue(docs);
    ...
    it('propaga errori del repository durante la ricerca semantica', async () => {
        (repo.searchDocumentSemantic as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('semantic fail'));
