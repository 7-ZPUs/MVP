...
const makeProcess = (id: number, uuid: string, metadata: { name: string; value: string }[] = []) => {
    ...
    return Process.fromDB(
        { id, documentClassId: 1, uuid, integrityStatus: 'UNKNOWN' },
        meta as any
    );
};
...
describe('SearchProcessUC', () => {
    ...
    it('passa stringa vuota al repository per ricerca senza filtro', () => {
        const procs = [
            makeProcess(1, 'uuid-1'),
            makeProcess(2, 'uuid-2'),
        ];
    ...
    it('preserva l\'ordine dei risultati restituito dal repository', () => {
        ...
        expect(results.map(r => r.getUuid())).toEqual(['uuid-aaa', 'uuid-bbb', 'uuid-ccc']);
    });
