...
describe("ProcessDAO", () => {
  ...
  beforeEach(() => {
    ...
    documentClassId = Number(
      db.prepare(
        ...
        .run(dipId, "dc-proc-uuid", "UNKNOWN", "Classe", "2026-01-01")
        .lastInsertRowid,
    );
  ...
  it("saves and retrieves process with metadata tree", () => {
    const entity = new Process(documentClassUuid, "proc-a", buildMetadata("v1"));
    ...
    expect(found?.getMetadata().findNodeByName("Versione")?.getStringValue()).toBe(
      "v1",
    );
  });
  ...
  it("gets by documentClassId and status and updates integrity", () => {
    const a = dao.save(new Process(documentClassUuid, "proc-c", buildMetadata("v1")));
    const b = dao.save(new Process(documentClassUuid, "proc-d", buildMetadata("v1")));
  ...
  it("computes aggregated integrity status", () => {
    const a = dao.save(new Process(documentClassUuid, "proc-e", buildMetadata("v1")));
    const b = dao.save(new Process(documentClassUuid, "proc-f", buildMetadata("v1")));

    expect(dao.getAggregatedIntegrityStatusByDocumentClassId(documentClassId)).toBe(
