...
describe("DocumentMapper", () => {
  ...
  it("maps persistence row to domain", () => {
    ...
    const entity = DocumentMapper.toDomain(row, metadataRows);
  ...
  it("uses UNKNOWN when status is missing or invalid", () => {
    ...
    expect(DocumentMapper.toDomain(missingStatus, metadataRows).getIntegrityStatus()).toBe(
      IntegrityStatusEnum.UNKNOWN,
    );
    expect(DocumentMapper.toDomain(invalidStatus, metadataRows).getIntegrityStatus()).toBe(
      IntegrityStatusEnum.UNKNOWN,
    );
  });
