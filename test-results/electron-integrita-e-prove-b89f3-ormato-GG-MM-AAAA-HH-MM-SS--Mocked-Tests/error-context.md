...
- Name: electron/integrita-e-provenienza/integrita-e-provenienza-mocked.spec.ts >> Integrita e Provenienza - Mocked >> [TS-199] Verificare che l'utente possa visualizzare la data e l'ora di inizio della verifica del DIP nel formato "GG/MM/AAAA HH:MM:SS"
- Location: e2e/electron/integrita-e-provenienza/integrita-e-provenienza-mocked.spec.ts:244:7
...
Locator: getByTestId('integrity-verification-start-date')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByTestId('integrity-verification-start-date')
...
```ts
  152 |   test(`[TS-218] Verificare che l'utente possa visualizzare il nome del documento all'interno del report di integrità`, async ({ page }) => {
  153 |     await installMockIpc(page);
  154 | 
  155 |     const aipDetailsPage = new AipDetailsPage(page);
  156 |     await aipDetailsPage.openDocumentDetailFromTree();
  157 | 
  158 |     await expect(page.getByRole('heading', { name: 'verbale.pdf' })).toBeVisible();
  159 |   });
  160 | 
  161 |   test(`[TS-219] Verificare che l'utente possa visualizzare lo stato della verifica (Valido / Non Valido) per il documento selezionato`, async ({ page }) => {
  162 |     await installMockIpc(page);
  163 | 
