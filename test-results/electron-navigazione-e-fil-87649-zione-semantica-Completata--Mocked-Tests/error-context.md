...
- Name: electron/navigazione-e-filtri-parte-1/dip-navigation-mocked.spec.ts >> Navigazione e Filtri Parte 1 - Mocked >> [TS-28] Verificare che l'utente possa visualizzare lo stato di indicizzazione semantica "Completata"
- Location: e2e/electron/navigazione-e-filtri-parte-1/dip-navigation-mocked.spec.ts:212:7
...
> 214 |     await expect(dipPage.semanticToggle).toBeHidden();
  ...
  308 |   });
  309 | 
  310 |   test(`[TS-42] Verificare che il sistema permetta di selezionare filtri per tipo documentale per un singolo tipo di documento`, async ({ page }) => {
  311 |     const dipPage = await gotoSearch(page);
  312 |     await dipPage.openFilterSection('Dati Generali');
  313 | 
  314 |     await dipPage.typeSelect.selectOption({ index: 1 });
```